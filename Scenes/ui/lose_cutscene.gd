...
# Lose cutscene that plays the defeat video using VLC plugin
...
func _ready() -> void:
	...
	var media = VLCMedia.load_from_file("res://videos/lose.mp4")
	if media:
		video_player.set_media(media)
		# Start playing
		video_player.play()
		_playing = true
		print("[LoseCutscene] VLC video started playing")
	else:
